'use client'

import { FC } from 'react'
import { Container } from '@/components/layout/Container'
import { getBusinessRatingSnapshot } from '@/constants/business'

export const TrustBadges: FC = () => {
  const businessRating = getBusinessRatingSnapshot()

  const badges = [
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
      ),
      title: 'Fully Certified & Insured',
      description: 'Complete liability and workers comp coverage',
    },
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" />
        </svg>
      ),
      title: 'CertainTeed ShingleMaster Premier',
      description: 'Factory-certified elite contractor',
    },
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
      title: '24/7 Emergency Service',
      description: 'Always available when you need us',
    },
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
      ),
      title: 'Flexible Financing',
      description: 'Easy payment plans available',
    },
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
        </svg>
      ),
      title: 'Free Inspections',
      description: 'No-obligation roof assessments',
    },
    {
      icon: (
        <svg className="w-12 h-12 text-accent-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
        </svg>
      ),
      title: `${businessRating.ratingValue} Star Rating`,
      description: `${businessRating.reviewCount} verified Google reviews`,
    },
  ]

  return (
    <section className="py-12 bg-primary-50 border-y border-primary-200">
      <Container>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {badges.map((badge, index) => (
            <div
              key={index}
              className="flex flex-col items-center text-center group hover:scale-105 transition-transform"
            >
              <div className="mb-3 group-hover:text-accent-700 transition-colors">
                {badge.icon}
              </div>
              <div className="font-bold text-sm text-primary-900 mb-1">
                {badge.title}
              </div>
              <p className="text-xs text-primary-600">
                {badge.description}
              </p>
            </div>
          ))}
        </div>
      </Container>
    </section>
  )
}
